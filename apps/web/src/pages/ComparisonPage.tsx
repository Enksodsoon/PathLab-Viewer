...
export function ComparisonPage() {
  ...
  const synchronize = useCallback((source: ComparisonMember, snapshot: ImageViewport) => {
    ...
    for (const targetId of panes) {
      ...
      if (!target || (target.slideId !== comparison.referenceSlideId && target.registration?.status !== 'ready')) continue
      const [centerX, centerY] = mapComparisonPoint([snapshot.centerX, snapshot.centerY], transform(source, comparison.referenceSlideId), transform(target, comparison.referenceSlideId))
  ...
  return <div className="comparison-shell">
    <header className="comparison-header"><Brand variant="library" /><div><strong>{comparison.name}</strong><span>{comparison.status} · {comparison.members.length} slides</span></div><button type="button" onClick={() => setLinked((value) => !value)}>{linked ? <LinkSimple /> : <LinkBreak />}{linked ? 'Views linked' : 'Views independent'}</button><button type="button" onClick={() => handles.current.forEach((handle) => handle.home())}><ArrowsClockwise /> Reset</button></header>
    ...
    <main className={`comparison-grid comparison-grid--${panes.length}`}>
      {panes.map((slideId, paneIndex) => {
        ...
        return <section className="comparison-pane" key={`${paneIndex}-${slideId}`}>
          <header><select aria-label={`Slide shown in pane ${paneIndex + 1}`} value={slideId} onChange={(event) => setPanes((current) => current.map((id, index) => index === paneIndex ? event.target.value : id))}>{comparison.members.filter((candidate) => !panes.includes(candidate.slideId) || candidate.slideId === slideId).map((candidate) => <option key={candidate.slideId} value={candidate.slideId}>{candidate.stain || 'Unspecified stain'} · {candidate.displayName}</option>)}</select><span className={aligned ? 'alignment-ready' : 'alignment-unavailable'}>{aligned ? (member.slideId === comparison.referenceSlideId ? 'Reference' : 'Aligned') : 'Not aligned'}</span>{panes.length > 2 ? <button aria-label={`Close ${member.displayName} pane`} onClick={() => setPanes((current) => current.filter((_, index) => index !== paneIndex))}><X /></button> : null}</header>
    ...
    {panes.length < Math.min(MAX_PANES, comparison.members.length) ? <button className="comparison-add-pane" onClick={() => { const next = comparison.members.find((member) => !panes.includes(member.slideId)); if (next) setPanes((current) => [...current, next.slideId]) }}><Plus /> Add pane</button> : null}
  </div>
}
