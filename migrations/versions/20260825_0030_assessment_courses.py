...
def upgrade() -> None:
    op.create_table(
        ...
        sa.CheckConstraint("status IN ('draft', 'active', 'archived')", name="ck_assessment_courses_status"),
        sa.CheckConstraint(
        ...
        sa.UniqueConstraint("organization_id", "course_code", name="uq_assessment_courses_org_code"),
    )
    op.create_index("ix_assessment_courses_organization_id", "assessment_courses", ["organization_id"])
    ...
    op.create_table(
        ...
        sa.Column("created_at", sa.DateTime(timezone=True), nullable=False),
        sa.Column("updated_at", sa.DateTime(timezone=True), nullable=False),
        sa.CheckConstraint("status IN ('active', 'withdrawn')", name="ck_course_enrollments_status"),
        sa.ForeignKeyConstraint(["organization_id"], ["organizations.id"], ondelete="CASCADE"),
    ...
    op.create_index("ix_assessment_course_enrollments_organization_id", "assessment_course_enrollments", ["organization_id"])
    op.create_index("ix_assessment_course_enrollments_course_id", "assessment_course_enrollments", ["course_id"])
    ...
    for index, cohort in enumerate(cohorts, start=1):
        ...
        connection.execute(
            sa.text(
                ...
                "(id, organization_id, name, course_code, semester, academic_year, scoring_method, description, "
                "opens_at, closes_at, status, schema_version, created_by_user_id, created_at, updated_at) "
                "VALUES (:id, :organization_id, :name, :course_code, 'Imported', NULL, 'percentage', "
                ":description, NULL, NULL, :status, 1, :created_by_user_id, :created_at, :updated_at)"
            ),
            {
                "id": course_id,
        ...
        connection.execute(
            sa.text(
                "UPDATE cohorts SET assessment_course_id = :course_id, section_code = :section_code "
                "WHERE id = :cohort_id"
            ),
        ...
        enrollments = connection.execute(
            sa.text(
                "SELECT organization_id, learner_id, status, created_by_user_id, created_at, updated_at "
        ...
        for enrollment in enrollments:
            connection.execute(
                sa.text(
                    ...
                    "(id, organization_id, course_id, learner_id, status, created_by_user_id, created_at, updated_at) "
                    "VALUES (:id, :organization_id, :course_id, :learner_id, :status, :created_by_user_id, :created_at, :updated_at)"
                ),
...
def downgrade() -> None:
    ...
    columns = ("closes_at", "opens_at", "location", "meeting_schedule", "description", "section_code", "assessment_course_id")
    if op.get_bind().dialect.name == "sqlite":
