...
Accepted inputs are valid two-dimensional RGB OME-TIFF files with `SizeZ=1` and `SizeT=1`. Classic TIFF and BigTIFF, either endianness, flat or pyramidal, tiled or striped, unsigned 8/16-bit samples, and JPEG/LZW/Deflate/uncompressed payloads are supported. Sixteen-bit samples map linearly to eight-bit output. A narrow compatibility path accepts legacy ImageJ converter output when the first IFD independently contains a valid interleaved 2D RGB image and ImageJ metadata declares one Z plane and one timepoint. It ignores malformed trailing page-chain entries instead of relaxing validation for plain TIFFs, stacks, or unsupported pixel formats.
