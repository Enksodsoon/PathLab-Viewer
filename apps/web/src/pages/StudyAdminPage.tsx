...
export function StudyAdminPage() {
  ...
  const create = async () => {
    ...
    try {
      await createStudyCourse({ packId, title: title.trim(), retentionDays: retention, learnerLimit })
  ...
  return <main className="study-admin">
    ...
    <section className="study-admin-heading"><div><span>Educational beta</span><h1>Study Coach</h1><p>Publish deterministic faculty guidance first. Local AI remains independently gated.</p></div><a href="/study" target="_blank" rel="noreferrer">Open learner entry</a></section>
    ...
    {loading ? <Loader label="Loading Study Coach…" /> : <>
      <section className="study-admin-card" aria-labelledby="new-course-heading">
        ...
        {packs.length ? <div className="study-admin-form">
          ...
          <button type="button" disabled={busy === 'create' || !title.trim()} onClick={() => void create()}><Plus aria-hidden="true" /> Create draft</button>
        </div> : <p>No Study Packs are available. Author and faculty-preview one in Forge, then publish it through the paired Viewer connection.</p>}
      </section>
      <section className="study-admin-courses" aria-labelledby="courses-heading"><h2 id="courses-heading">Courses</h2>{courses.length ? courses.map((course) => <article key={course.id}>
        <div><span className={`study-course-state ${course.status}`}>{course.status}</span><h3>{course.title}</h3><p>{course.redeemed} redeemed · {course.invitations} invitations · {course.retentionDays}-day retention</p><p>Device checks: {course.readiness.ready} ready, {course.readiness.fallback} deterministic fallback</p></div>
        <div className="study-course-controls">
          ...
          {course.status === 'preparation' ? <button type="button" disabled={busy !== '' || course.invitations < 1} onClick={() => void transition(course, 'activate')}><Play aria-hidden="true" /> Activate deterministic course</button> : null}
