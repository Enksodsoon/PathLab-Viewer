...
export function AssessmentReportPage({ embedded = false }: { embedded?: boolean }) {
  ...
  const aggregateOnly = Boolean(results && results.summary.responses > 0 && individuals.length === 0)
    ...
    <main className={`assessment-main assessment-report-page assessment-responses-page${embedded ? ' assessment-report-page--embedded' : ''}`}>
      ...
      <div className="assessment-responses-navigation">
        ...
        <div className="assessment-report-export-actions" aria-label="Export report">
          ...
          {aggregateOnly ? <button type="button" disabled><ImageSquare aria-hidden="true" /> Visual</button> : <a href={`data:image/svg+xml;charset=utf-8,${encodeURIComponent(visualSvg)}`} download={`${fileStem(draft.title)}-visual-report.svg`}><ImageSquare aria-hidden="true" /> Visual</a>}
        </div>
      </div>

      {!results ? <EmptyReport draft={draft} /> : null}

      {results && aggregateOnly && (view === 'overall' || view === 'questions') ? <section aria-label="Aggregate response summary">
        <h2>Response summary</h2>
        <p>Individual records are unavailable for these responses. The totals below include anonymous responses. Score percentages, medians, and learner distributions are unavailable.</p>
        <dl><dt>Average points</dt><dd>{formatPoints(results.summary.averagePoints)}</dd><dt>Responses</dt><dd>{results.summary.responses}</dd></dl>
        {questions.map((question) => <article key={question.itemId}><h3>{question.item.prompt || `Question ${question.index + 1}`}</h3><dl><dt>Responses</dt><dd>{question.summary.responseCount}</dd><dt>Average points</dt><dd>{formatPoints(question.summary.averagePoints)}</dd></dl></article>)}
      </section> : null}

      {results && !aggregateOnly && view === 'overall' ? <div className="assessment-responses-summary">
      ...
      {results && !aggregateOnly && view === 'questions' ? <section className="assessment-response-question-stack" aria-label="Question response summaries">{questionResponses.map((question) => renderQuestionResponse(question))}</section> : null}
