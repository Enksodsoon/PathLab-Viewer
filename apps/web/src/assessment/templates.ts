...
export const assessmentTemplates: AssessmentTemplate[] = [
  ...
  template('slide-seminar', 'Slide seminar', 'A guided case discussion with observations before synthesis.', [
    { id: 'seminar-observe', title: 'Observe', items: [{ ...item('seminar-observation', 'short-answer', 'Describe the key morphologic findings.'), manual: true }] },
  ...
  template('confidence-check', 'Confidence check', 'A compact answer and confidence pair.', [
    { id: 'confidence-section', title: 'Answer and reflect', items: [item('confidence-answer', 'multiple-choice', 'Select your diagnosis.'), item('confidence-rating', 'rating', 'How confident are you?')] },
  ]),
  ...
  template('exit-ticket', 'Exit ticket', 'A brief end-of-session knowledge and reflection check.', [
    { id: 'exit-section', title: 'Before you leave', items: [item('exit-answer', 'short-answer', 'Name one finding you can now recognize.'), { ...item('exit-reflection', 'short-answer', 'What remains unclear?'), manual: true }] },
  ]),
]
...
export function parseAssessmentPaste(source: string): PasteParseResult {
  ...
  lines.forEach((raw, index) => {
    ...
    const type = ({ mc: 'multiple-choice', dropdown: 'multiple-choice', rating: 'rating', text: 'short-answer', short: 'short-answer', paragraph: 'short-answer' } as const)[normalizedKind as 'mc']
    if (!type) { errors.push({ line: index + 1, message: 'Use mc, rating, or text.' }); return }
    ...
    if (normalizedKind === 'paragraph') created.manual = true
