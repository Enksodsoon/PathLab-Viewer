...
export function ComparisonPage() {
  ...
  return <div className="comparison-shell">
    ...
    <div className="comparison-workstation" data-tray-open={trayOpen}><aside className="comparison-tray" aria-label="Case slides"><strong>Case slides <span>{comparison.members.length}</span></strong>{comparison.members.map((member) => <button type="button" key={member.slideId} disabled={!!correction} data-active={panes.includes(member.slideId)} data-current={panes[activePane] === member.slideId} aria-current={panes[activePane] === member.slideId ? 'true' : undefined} onClick={() => { const existing = panes.indexOf(member.slideId); if (existing >= 0) { setActivePane(existing); if (maximizedPane !== null) setMaximizedPane(existing); return } selectPaneSlide(Math.min(activePane, panes.length - 1), member.slideId) }}><img src={member.thumbnailUrl} alt="" loading="lazy" /><span><b>{member.stain || 'Unspecified'}</b><small>{member.displayName}</small></span></button>)}</aside>
